...
### Architecture Builder Edge Routing Rules
The `ArchitectureBuilder` component follows 8 strict routing rules for its orthogonal edges:
1. Segments must be strictly horizontal or vertical.
2. Edges must exit/enter perpendicular to the handle side.
3. All step/smoothstep edges render via `buildPolylinePath(pinnedWaypoints)`.
4. Waypoints are stored lazily (empty on connect, computed on drag).
5. Pin first/last waypoints every render to keep exits perpendicular.
6. Do NOT pass `selected` at the top level of an edge object (causes drag issues).
7. Segment drag is axis-locked and snap-aware.
8. `waypoints[]` is the only source of truth for routing.
