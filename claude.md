...
4. **Waypoints are stored lazily but persist once set.** `onConnect` writes `waypoints: []`; `CustomEdge` routes live via `computeAutoWaypoints` until the user drags a segment. Once waypoints exist (manual routing), they PERSIST even when endpoints or parent nodes are moved. `onEdgeUpdate` (endpoint reconnect) and `onNodeDragStop` (node move) now retain existing waypoints, allowing the first/last segments to stretch while keeping manual routing intact. Waypoints must only be cleared via the "Clear Path" context menu action.
