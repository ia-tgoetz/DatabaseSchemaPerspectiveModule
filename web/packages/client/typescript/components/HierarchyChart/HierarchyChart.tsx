...
const getVariantColor = (variant: string) => {
    switch(variant) {
        ...
        case 'pending': default: return 'var(--container)'; 
    }
};
...
const mapConnectionLineType = (type: string) => {
    switch (type) {
        ...
        case 'bezier': case 'default': default: return ConnectionLineType.Bezier;
    }
};
...
const HierarchyNode = ({ data }: any) => {
    ...
    const baseStyles = {
        padding: '10px 20px', border: '1px solid var(--border)', borderRadius: '5px', 
        backgroundColor: getVariantColor(data.variant), color: 'var(--neutral-90)',
        textAlign: 'center' as const, minWidth: '150px', ...customStyles 
    };
...
export const HierarchyChart = observer((props: ComponentProps<HierarchyChartProps>) => {
    ...
    const displayEdges = edges.map((edge: any) => ({
        ...edge,
        style: { stroke: edge.selected ? 'var(--callToAction)' : '#888', strokeWidth: edge.selected ? 3 : 2 },
        markerEnd: { type: MarkerType.ArrowClosed, width: 20, height: 20, color: edge.selected ? 'var(--callToAction)' : '#888' }
    }));
